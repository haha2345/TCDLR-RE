function [L,S,err,iter,convergence] = trpca_fast(X,lambda,fun,p,l,fast_p,opts)
...
for iter = 1 : max_iter
    ...
    % update L
%     tic
%    [L] = FRSVT_tensor_test(-S+X-Y/mu,1/mu,fun,p,false,l,fast_p);

   [L] = FRSVT_tensor_sim(-S+X-Y/mu,1/mu,fun,p,false,l,fast_p);
 %[L,~] = FRSVT_tensor(-S+X-Y/mu,1/mu,fun,p,true);
    %     toc
%     [L,tnnL] = prox_fast(-S+X-Y/mu,50,W);
% toc
    % update S
...
% [L,~]=Helper_gen(X,tau,l,p,L,fun,normpar);
% obj = tnnL+lambda*norm(S(:),1);
err = norm(dY(:));
